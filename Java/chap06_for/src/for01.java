...
public class for01 {
	public static void main(String[]args) {
		...
		/*
		 for문은 프로그래밍에서 반복작업을 수행하기 위해사용되는 제어구조
		 특정한 조건이 반족하는 동안 코드 블록을 반복 실행하는데 사용
		 대부분의 프로그래밍 언어에서 for문이 가진 구조
		 for(초기화;조건;증감){
		 //조건이 참일 경우 반복적으로 실행할 코드
		  * }
		  * 초기화 : 반복문이 시작될 때 한번만 실행되는 부분으로 반복 변수를 초기화하는 역할을 한다.
		...
		for(int i =1; i<=5; i++) {
			sum1 += i;
			System.out.println("1~5까지의 합: " +sum1);
		}
